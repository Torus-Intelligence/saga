---
name: saga
description: Author, adapt, and run Saga scenario tests for backend or agent workflows using @torus-oss/saga. Use when a coding agent needs to add or debug YAML Saga fixtures, wire event injectors to production code through DI, define typed effect matchers, inspect trajectory failures, configure fail-stop/auto-PR/ticket outcome adapters, or add persona simulation with scrape/fingerprint/discriminator/evolve modules.
---
